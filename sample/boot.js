async function boot(GIS) {
  ...
  //Add new map
  window.map = new ESRI.Map({
    basemap: 'osm'
  })

  //Add new mapView
  window.mapView = new ESRI.MapView({
    map: map,
    container: 'mapDiv',
    center: [118.8690154, -1.8085194],
    zoom: 4
  })
  ...
  setWindowVariables()
  //All localstorage items will be included here
  setStartLocalStorage()
  //Execute functions when view finish loaded
  mapViewWhen()
  //Execute functions when view clicked
  mapViewClick()
  //Context menu hover for analyzing
  analyzeHover(["radius", "drivingtime", "drivingdistance", "manual"])
  //Context menu hover for measurement area and length
  measurementHover(["polygon", "polyline"])
  //InputFilter for filtering input 
  inputFilter()
  //Target extent of mapView
