  ...
  create() {
    ...
    for (let i = 0; i < radius.length; i++) {
      ...
      let circleSymb = new ESRI.SimpleFillSymbol({
        ...
        color: [Math.floor(Math.random() * 255 + 1), Math.floor(Math.random() * 255 + 1), Math.floor(Math.random() * 255 + 1), 0.2]
      });
    ...
    // for (let key in this.Radius) {
    //   console.log(key)
    //   console.log(this.Radius[key])
...
export class DriveTime {
  ...
  run(callback) {
    let fillSymbol = {
      type: "simple-fill",
      color: [Math.floor(Math.random() * 255 + 1), Math.floor(Math.random() * 255 + 1), Math.floor(Math.random() * 255 + 1), 0.1],
      outline: {
        style: "long-dash",
        color: [
          Math.floor(Math.random() * 255 + 1),
