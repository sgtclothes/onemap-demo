export class Radius {
  constructor(map, mapView, latitude, longitude) {
    ...
    this.FeatureLayer = new ESRI.FeatureLayer(
      "http://tig.co.id/ags/rest/services/BPS/BPS_2016_DESA/MapServer/3"
    );
  }
  ...
  create() {
    ...
    query.outFields = ["POPULASI", "PROVINSI"];
    ...
    this.FeatureLayer.queryFeatures(query).then(function(results) {
      ...
      totalPopulasi = totalPopulasi
        .toString()
        .replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1,");
      let prov = results.features[0].attributes.PROVINSI;
      circleGraphic.attributes = { POPULASI: totalPopulasi, PROVINSI: prov };
      ...
      arrayUtils.forEach(
        results.features[0].layer.defaultPopupTemplate.fieldInfos,
        function(field) {
          if (field.fieldName == "POPULASI" || field.fieldName == "PROVINSI") {
...
export class Catchment {
  ...
  setUrlFeaturesLayer(url, callback) {
    this.FeatureLayer = new ESRI.FeatureLayer(url);
    callback();
  }

  runFeaturesLayer(graphicsLayers) {
    let query = new ESRI.Query();
    query.returnGeometry = true;
    query.geometry = this.Geometry.extent;
    query.outFields = ["POPULASI", "PROVINSI"];
    this.FeatureLayer.queryFeatures(query).then(function(resultQuery) {
      let totalPopulasi = 0;
      resultQuery.features.forEach(function(feature) {
        totalPopulasi += feature.attributes.POPULASI;
      });
      ...
      totalPopulasi = totalPopulasi
        .toString()
        .replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1,");
      let prov = resultQuery.features[0].attributes.PROVINSI;
      graphicsLayers.attributes = { POPULASI: totalPopulasi, PROVINSI: prov };

      let popupContent = {
        content: "<table class='esri-widget__table'>"
      };
      arrayUtils.forEach(
        resultQuery.features[0].layer.defaultPopupTemplate.fieldInfos,
        function(field) {
          if (field.fieldName == "POPULASI" || field.fieldName == "PROVINSI") {
            popupContent.content +=
              "<tr><th class='esri-feature__field-header'>" +
...
export class Database {
  ...
  replaceData(currentData, namefile, storage, createdBy, result, color) {
    ...
    updateDatabaseTable(namefile, result, storage, JSON.stringify(color));

    function updateDatabaseTable(name, data, storage, color) {
      let tableName = name + "_" + storage + "_" + created_by;
      $.ajax({
        url: "content/deleteTable.php",
        method: "POST",
        data: {
          tableName: tableName
        },
    ...
    $(document).delegate("a[name='delete-user-poi']", "click", function() {
      ...
      let confirmBox = new ConfirmBox(
        ...
        function() {
          ...
          let indexNameFile = namefile.indexOf(
            $(self)
              .siblings("label")
              .text()
          );
          ...
          if (
            ...
            $(self)
              ...
              .remove();
            console.log($(self).parents(".treeview-user"));
          } else {
            $(self)
              .parent()
              .remove();
