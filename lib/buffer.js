...
export class LocalStorage {
  ...
  checkData() {
    console.log("Read Local Storage Data");
    console.log(JSON.parse(localStorage.getItem("namefile")));
...
export class ConvertCSV {
  ...
  setupDropZone() {
    ...
    on(x, "drop", function(event) {
      ...
      x.style.display = "none"; //Hide black modal
    });
  }
  ...
  processCSVData(data, rendered, val) {
    ...
    csvStore.fetch({
      onComplete: function(items) {
        ...
        if (rendered == true) {
          randomColor = JSON.parse(localStorage.getItem("color"))[val];
        } else {
          for (let i = 0; i < 3; i++) {
        ...
        if (!rendered) {
          ...
          if (
            storeToLocalStorage.doubleData("createdBy", created_by) == false
          ) {
            storeToLocalStorage.insert("createdBy", created_by);
          }
...
export class Viewer {
  ...
  addTreeview(nameFile, length) {
    let self = this;
    let username = document.getElementById("username").innerHTML;
    let createdBy = JSON.parse(localStorage.getItem("createdBy"));
    if (createdBy !== null && createdBy.includes(created_by)) {
      $.get("template/addPOI.php", function(poi) {
        $(".user-poi-" + created_by).append(poi);
        $(".user-poi-" + created_by + " .li-user-poi").attr(
          "class",
          "li-user-poi-" + created_by + "-" + length
        );
        $(".li-user-poi-" + created_by + "-" + length + " input.poi").attr(
          "class",
          "poi-" + created_by + "-" + length
        );
        $(
          ".li-user-poi-" +
            created_by +
        ...
        $(".li-user-poi-" + created_by + "-" + length + " label.poi").attr(
          "class",
          "poi-" + created_by + "-" + length
        );
        $(
          ".li-user-poi-" +
            ...
            "-" +
            length
        ).attr("for", "poi-" + created_by + "-" + length);
        $(
          ".li-user-poi-" +
            created_by +
            "-" +
        ...
        let item = $(
          ".li-user-poi-" + created_by + "-" + length + " input"
        ).attr("class");
        let val = $(
          ".li-user-poi-" + created_by + "-" + length + " input"
        ).attr("value");
      ...
      $.get("template/treeview.php", function(data) {
        $("#tree-viewer").append(data);
        $(".user").attr("class", "user-" + created_by);
        $(".master-select-all-poi").attr(
          "class",
          "master-select-all-poi-" + created_by
        );
        $(".label-user").attr("for", "master-select-all-poi-" + created_by);
        $(".label-user").attr("class", "label-user-" + created_by);
        $(".label-user-" + created_by).text(username);
      });
      $.get("template/addPOI.php", function(poi) {
        $(".user-poi").attr("class", "user-poi-" + created_by);
        $(".user-poi-" + created_by).append(poi);
        $(".user-poi-" + created_by + " .li-user-poi").attr(
          "class",
          "li-user-poi-" + created_by + "-" + length
