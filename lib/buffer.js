export class Overlap {
  ...
  create() {
    ...
    graphicsLayer.when(function () {
      map.ObjMap.extent = graphicsLayer.fullExtent;
    });
...
export class Pointing {
  ...
  render() {
    ...
    EsriRequest(url, {
      ...
      let popupContent = {
        title:
          response.data.address.Subregion + ", " + response.data.address.Region,
        content: response.data.address.LongLabel
      };
      pointGraphic.popupTemplate = popupContent;
    });

    this.MapView.graphics.add(pointGraphic);

    if (this.PosLongitude === null && this.PosLatitude === null) {
      this.MapView.goTo({
        target: pointGraphic,
        zoom: 13
      });
      ...
      this.MapView.goTo({
        target: [this.PosLongitude, this.PosLatitude],
        zoom: 13
      });
    }
  }
  positionFixing(lat, lon) {
    this.PosLatitude = lat;
    this.PosLongitude = lon;
  }
}
...
export class Radius {
  ...
  create(callback) {
    ...
    this.FeatureLayer.queryFeatures(query).then(function (results) {
      ...
      graphicsLayer.value = self.Value;
      graphicsLayer.options = self.Options;
      graphicsLayer.anly = self.Anly;
      self.Map.add(graphicsLayer);
      graphicsLayer.add(circleGraphic);
      self.graphicsLayer = graphicsLayer;
      self.MapView.goTo({
        target: circleGraphic
      });
      callback();
    });
  }
  ...
  setOptions(value, options, latitude, longitude, unit, distance) {
    this.Value = value;
    this.Options = parseInt(options);
    let anly = {};
    anly.latitude = latitude;
    anly.longitude = longitude;
    anly.distance = distance;
    anly.unit = unit;
    this.Anly = JSON.stringify(anly);
  }
}
...
export class DriveTime {
  ...
  render(mapView) {
    ...
    graphicsLayer.value = this.Value;
    graphicsLayer.options = this.Options;
    graphicsLayer.anly = this.Anly;
...
export class QueryShape {
  ...
  create(callback) {
    let symb = new ESRI.SimpleFillSymbol({
      style: "solid",
      outline: {
        style: "long-dash",
        color: [
    ...
    let shapeGraphic = new ESRI.Graphic({
      geometry: this.Shape,
      symbol: symb
    });
    ...
    this.FeatureLayer.queryFeatures(query).then(function (results) {
      ...
      popupContent.content += "</table>";
      shapeGraphic.popupTemplate = popupContent;
      let graphicsLayer = new ESRI.GraphicsLayer();
      graphicsLayer.value = self.Value;
      graphicsLayer.options = self.Options;
      graphicsLayer.anly = self.Anly;
      graphicsLayer.customShape = true;
      self.Map.add(graphicsLayer);
      graphicsLayer.add(shapeGraphic);
      self.graphicsLayer = graphicsLayer;
      self.MapView.goTo({
        target: shapeGraphic
      });
...
export class ConvertCSV {
  constructor(map, mapView, pointColor) {
    ...
    this.TempData = [];
    this.CreatedBy = [];
    this.latFieldStrings = ["lat", "latitude", "y", "ycenter", "property_lat"];
    this.longFieldStrings = ["lon", "long", "longitude", "x", "xcenter", "property_lng"];
  }
  ...
  processCSVData(data, rendered, val, alias, disablePopup) {
    console.log(data);
    let self = this;
    ...
    csvStore.fetch({
      onComplete: function (items) {
        ...
        let buttonRemove = $(".button-remove-filter");
        $(buttonRemove).show();
  ...
  buildInfoTemplate(popupInfo, alias) {
    ...
    let popupLayout = JSON.parse(localStorage.getItem("popupLayout"));
    let json = {
      content: popupLayout
    };
    console.log(document.getElementsByClassName("popupFilter"));
...
export class Viewer {
  ...
  selectItem() {
    ...
    function setAllItem(className) {
      ...
      //Comment code, there is a bug here -----
      // for (let key in val) {
